...
class CommentListBloc extends Bloc<CommentListEvent, CommentListState> {
  ...
  CommentListBloc({
    ...
    on<CommentListEvent>((event, emit) async {
      if (event is CommentListStartedEvent || event is CommentListRefreshEvent) {
        emit(CommentListLoading());
        try {
          final List<CommentEntity> comments = await commentRepository.getAll(productId: productId);
          emit(CommentListSuccess(comments: comments));
        } catch (e) {
          emit(CommentListError(appException: e is AppException ? e : AppException()));
        }
      }
    });
  }
